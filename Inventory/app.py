                        ...
                        for O in Order:
                            ...
                            try:
                                if(O.Products_id==Entrada.Products_id):
                                    ...
                                    print(REMI)
                                    print(Entrada.Number_remision)
                                    if str(Entrada.Number_remision) in REMI :
                                            REMI.remove(str(Entrada.Number_remision))

                                    R.Remision = '+'.join(REMI)
...
def Tickets_work():
    ...
    if request.method == 'POST' and Ticketform.validate():
        ...
        Order_new = Buy_order.query.filter(Buy_order.N_compra == Ticketform.N_Orde_Comprar.data).all()
        ...
        if (not flag):
            ...
            Requisi_new = Requisition.query.filter(Requisition.N_compra==Ticketform.N_Orde_Comprar.data).all()
            ...
            try:
                ...
                if (int(Ticketform.amount.data) == Requisi.amount and (Ticketform.Identifier_P.data ==  Buy.Products_id) ):
                    ...
                    Buy_order.query.filter(Buy_order.N_compra == Ticketform.N_Orde_Comprar.data).filter(Buy_order.Products_id==Ticketform.Identifier_P.data).update({"State": "Recibido"})
                    db.session.commit()
                    ...
                    Buy_order.query.filter(Buy_order.N_compra == Ticketform.N_Orde_Comprar.data).filter(Buy_order.Products_id==Ticketform.Identifier_P.data).update({"State": str(int(Requisi.amount) - int(Ticketform.amount.data))})
                    db.session.commit()

                elif(str(Buy.State) != "Recibido" and str(Buy.State) != "No Recibido"):
                    Buy_order.query.filter(Buy_order.N_compra == Ticketform.N_Orde_Comprar.data).filter(Buy_order.Products_id==Ticketform.Identifier_P.data).update({"State": str(int(Buy.State) - int(Ticketform.amount.data))})
                    db.session.commit()
                ...
                if(not flagtwo):
                    ...
                    Requisition.query.filter(Requisition.Products_id == Ticketform.Identifier_P.data).filter(Requisition.N_compra == Ticketform.N_Orde_Comprar.data).filter(Requisi.N_Requisition == Requisi.N_Requisition).update({"Remision":Requisi.Remision + "+"+ Ticketform.Number_remision.data})
                    db.session.commit()
            except Exception as e :
                print(e)
    ...
    if request.method == 'POST' and Departuresform.validate():
        ...
        try:
            ...
            if (int(Product_now.balance_now) == 0):
                ...
                Despartur = Departures(date= Departuresform.Date.data,amount = Departuresform.amount.data, Number_vale= Departuresform.N_vale.data,builder=Departuresform.builder.data,Dest=Departuresform.Destine.data,Products_id=Departuresform.Identifier_P.data)
